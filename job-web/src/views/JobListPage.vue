  ...
  methods: {
    ...
    reHandleFilter(options) {
      ...
      this.listQuery.experienceIds =
        options.expIds.indexOf(null) === -1 ? options.expIds : [];
      this.listQuery.companyScaleIds =
        options.scaleIds.indexOf(null) === -1 ? options.scaleIds : [];
      this.listQuery.companyDefineIds =
        options.defineIds.indexOf(null) === -1 ? options.defineIds : [];
    ...
    refreshInpCity(options) {
      ...
      for (let i = 0; i < cityValues.length; i++) {
        historyCityValues.push(
          cityValues[i].endsWith("市")
            ? cityValues[i].substr(0, cityValues[i].length - 1)
            : cityValues[i]
        );
      }
      ...
      let optionsAndHistoryArr = this.getArrDifference(
        historyCityValues,
        optionsCityValues
      );
    ...
    formatHistoryForm() {
      ...
      this.historyForm.defineIds = this.arrCopy(
        this.listQuery.companyDefineIds
      );
