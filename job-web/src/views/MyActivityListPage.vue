      ...
      >
        <b-media right-align vertical-align="center">
          <div class="d-flex align-items-center">
            ...
            <span
              class="text-small ml-4 text-warning"
              style="min-width: 90px"
              v-if="getTime(activity.startTime, activity.finishTime) >= 0"
            >
              <svg-icon icon-class="timer-down" />
              {{
                getTime(activity.startTime, activity.finishTime) === 0
                  ? "即将结束"
                  : getTime(activity.startTime, activity.finishTime) +
                    "天后结束"
              }}
            </span>
  ...
  methods: {
    ...
    getTime(startTime, finishTime) {
      if (new Date(startTime) - new Date() > 0) {
        ...
        return Math.floor(
          (new Date(finishTime) - new Date()) / 24 / 60 / 60 / 1000
        );
      }
    },
  },
