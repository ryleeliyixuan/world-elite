    ...
    <div class="main-container">
      ...
      >
        ...
        >
          ...
          >
            <div class="company-name" :ref="item.name">
              {{ item.name }}
              <span class="company-count">{{
                "共" + companyList[item.id].length + "家企业"
              }}</span>
            </div>
            <el-button
              :underline="false"
    ...
    this.$refs.scrollbar.wrap.onscroll = () => {
      if (!this.timer) {
        ...
        for (let i = 0; i < this.industryList.length; i++) {
          ...
          if (
            card &&
            this.$refs.scrollbar.wrap.scrollTop <= card[0].offsetTop
          ) {
