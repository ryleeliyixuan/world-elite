<template>
  <div class="app-container">
    <div class="title">
      {{ this.$route.query.id ? "活动编辑" : "活动发布" }}
    </div>
    <div class="content-container">
      <div class="line">
        <div class="name">活动名称<span>*</span></div>
        <el-input
          placeholder="请输入活动名（最多20字）"
        ...
        <div class="option-container" @click="onOverseasStudent">
          <svg-icon
            v-if="activityForm.onlyOverseasStudent !== '1'"
            icon-class="unselected"
            style="margin: 0 3px 0 14px"
          ></svg-icon>
          <svg-icon
            v-else
            icon-class="selected"
        ...
        >
          <svg-icon
            icon-class="questionMask"
            style="width: 10px; height: 11px; margin-left: 4px"
          ></svg-icon>
        </el-tooltip>
      </div>
      <div class="line">
        <div class="name" style="align-self: flex-start">
          <el-tooltip
            class="item"
            effect="dark"
            content="资料仅用作审核，不会公开或用于其他用途。"
            placement="top"
          >
            <svg-icon
              icon-class="questionMask"
              style="width: 10px; height: 11px; margin: 8px 4px"
            ></svg-icon>
          </el-tooltip>
          主办方类型<span>*</span>
        </div>
        <div class="value-container">
          <div class="value-line-container">
            <div class="option-container" @click="onOrganizerType('1')">
              <svg-icon
                v-if="activityForm.organizerType !== '1'"
                icon-class="unselected"
            ...
            <div
              ...
              @click="onOrganizerType('2')"
              style="margin-left: 21px"
            >
              <svg-icon
                v-if="activityForm.organizerType !== '2'"
                icon-class="unselected"
                style="margin-right: 3px"
              ></svg-icon>
              <svg-icon
            ...
            <div
              class="option-container"
              @click="onOrganizerType('3')"
              style="margin-left: 21px"
            >
              <svg-icon
                v-if="activityForm.organizerType !== '3'"
                icon-class="unselected"
                style="margin-right: 3px"
              ></svg-icon>
              <svg-icon
          ...
          <div v-if="activityForm.organizerType === '2'">
            <div class="line">
              <div class="name" style="width: auto; align-self: flex-start">
                营业证明<span>*</span>
              </div>
              <el-upload
              ...
              >
                <el-image
                  v-if="businessLicenseUploadPicOptions.localUrl"
      ...
      <div class="line">
        <div class="name">活动城市/线上<span>*</span></div>
        <el-cascader
          placeholder="城市"
      ...
      <div class="line">
        <div class="name">报名时间<span>*</span></div>
        <el-date-picker
      ...
      <div class="line">
        <div class="name">活动时间<span>*</span></div>
        <el-date-picker
      ...
      <div class="line">
        <div class="name" style="align-self: flex-start">
          <el-tooltip class="item" effect="dark" placement="top">
            <div slot="content">
              若报名无需审核，学生报名即可参加；若需<br />要审核，可在活动报名管理信息页进行审核<br />操作。
            </div>
            <svg-icon
              icon-class="questionMask"
              style="width: 10px; height: 11px; margin: 8px 4px"
      ...
      <div class="line">
        <div class="name">活动报名表<span>*</span></div>
        <div
          v-if="this.$route.query.id"
          class="edit-button"
          @click="onPreviewRegistration"
        >
          {{ applyTableTitle }}
        </div>
        <div v-else style="display: flex; align-items: center">
          <div
            v-if="applyTableTitle"
            :class="['edit-button', { 'edit-button-disabled': useTemplate }]"
          ...
          <div
            v-else
            :class="['add-button', { 'add-button-disabled': useTemplate }]"
            @click="!useTemplate && onAddRegistrationTemplate()"
          >
          ...
          >
            <div class="template-settings-container">
              <div class="line1">
                模板管理
                <span
                  >{{ registrationTemplateList.length }}/{{
                    templateCount.maxCount
                  }}</span
                >
                <el-tooltip class="item" effect="dark" placement="top">
                  <div slot="content">
                    最多可保存{{ templateCount.maxCount }}份报名表模板
                  </div>
                  <svg-icon
                    icon-class="questionMask"
                    style="width: 10px; height: 11px; margin: 4px"
                  ></svg-icon>
                </el-tooltip>
              </div>
              <div class="line2" v-for="item in registrationTemplateList">
                ...
                <div class="operate-container">
                  <svg-icon
                    icon-class="template-edit"
                    clickable
                    @click="onTemplateEdit(item)"
                  ></svg-icon>
                  <svg-icon
                    icon-class="template-delete"
                    clickable
                    @click="onTemplateDelete(item)"
                  ></svg-icon>
                </div>
              </div>
              <div
                class="line3"
                v-if="registrationTemplateList.length < templateCount.maxCount"
                @click="onTemplateAdd"
              >
                添加模板
                <svg-icon
                  icon-class="template-add"
                  clickable
            ...
            <svg-icon
              slot="reference"
              :icon-class="useTemplate ? 'settings-enable' : 'settings-disable'"
              style="width: 17px; height: 17px; vertical-align: -4px"
            ></svg-icon>
          </el-popover>
        </div>
      </div>
      <div class="poster-container">
        <div class="line" style="margin-top: 0">
          <div class="name" style="width: auto">活动海报<span>*</span></div>
        ...
        >
          <el-image
            v-if="posterUploadPicOptions.localUrl || activityForm.poster"
    ...
    <el-dialog
      ...
      width="445px"
    >
      <el-checkbox v-model="saveDraft"
        >将已填内容保存至草稿，下次点击发布活动可恢复</el-checkbox
      >
      <div class="button-container">
...
export default {
  ...
  data() {
    return {
      ...
      businessLicenseUploadPicOptions: {
        // 营业执照上传
        loading: false,
        action: "",
        params: {},
    ...
    activityTime(value) {
      if (value && value.length === 2) {
        ...
        this.registrationTimeOption = {
          disabledDate: (time) => {
            return (
              time.getTime() < Date.now() || time.getTime() > value[1].getTime()
            );
          },
        };
      } else {
    ...
    registrationTime(value) {
      if (value && value.length === 2) {
        ...
        this.activityTimeOption = {
          disabledDate: (time) => {
            return (
              time.getTime() < value[0].getTime() ||
              time.getTime() < value[1].getTime()
            );
          },
        };
      } else {
  ...
  created() {
    ...
    if (applyObject) {
      ...
      this.activityForm.questionnaireId = this.useTemplate
        ? this.templateId
        : this.applyTableId;
      ...
      // 存在报名表id 或 报名模板  0：报名表  1：报名模板
      if (this.activityForm.questionnaireType === "0") {
        // 报名表
        this.$axios
          .get(`/activity-questionnaire/${this.activityForm.questionnaireId}`)
          .then((response) => {
            this.applyTableTitle = response.data.title;
            this.applyTableId = response.data.id;
            this.applyTable = response.data;
          });
      } else if (this.activityForm.questionnaireType === "1") {
        // 报名模板
        this.$axios
          .get(`/questionnaire-template/${this.activityForm.questionnaireId}`)
          .then((response) => {
  ...
  methods: {
    ...
    handlerActivity(activityForm) {
      if (activityForm) {
        ...
        this.activityTime = [
          new Date(activityForm.activityStartTime),
          new Date(activityForm.activityFinishTime),
        ];
        this.registrationTime = [
          new Date(activityForm.registrationStartTime),
          new Date(activityForm.registrationFinishTime),
        ];
    ...
    // 点击留学生
    onOverseasStudent() {
      this.activityForm.onlyOverseasStudent =
        this.activityForm.onlyOverseasStudent === "0" ? "1" : "0";
    },
    ...
    beforeBusinessLicenseUpload(file) {
      return new Promise((resolve, reject) => {
        if (checkPicSize(file)) {
          ...
          this.businessLicenseUploadPicOptions.localUrl = URL.createObjectURL(
            file
          );
          getUploadPicToken(file.name)
            .then((response) => {
              ...
              this.businessLicenseUploadPicOptions.fileUrl =
                data.host + "/" + data.key;
              resolve(data);
            })
            .catch((error) => {
    ...
    onEditRegistrationTemplate() {
      ...
      this.$router.push({
        path: "/activity/apply/table",
        query: { id: this.applyTableId, type: "0" },
      });
    },
    ...
    checkForm() {
      ...
      if (!this.activityForm.title) {
        ...
        !this.activityForm.organizerInfoForm.organizerName &&
        !this.activityForm.organizerInfoForm.school
      ) {
        message = "请输入组织名称";
      } else if (
        this.activityForm.organizerType === "2" &&
        !this.activityForm.organizerInfoForm.businessLicenseUrl
      ) {
        message = "请上传营业执照";
        ...
        !this.activityForm.activityStartTime ||
        !this.activityForm.activityFinishTime
      ) {
        message = "请选择活动时间";
      } else if (
        !this.activityForm.registrationStartTime ||
        !this.activityForm.registrationFinishTime
      ) {
        message = "请选择报名时间";
      } else if (
        this.activityForm.auditType === "1" &&
        !this.activityForm.numberLimit
      ) {
        message = "请输入限制报名人数";
    ...
    onRestoreDraft() {
      ...
      if (activityForm) {
        ...
        if (
          ...
          activityForm.registrationFinishTime
        ) {
          this.registrationTime = [
            new Date(activityForm.registrationStartTime),
            new Date(activityForm.registrationFinishTime),
