  ...
  <loading :loading="loading">
    <div class="app-container">
      <div class="menu-button-container">
        ...
        <div>
          <el-button size="small" @click="dialogVisible3 = true" type="primary"
            >上传简历</el-button
          >
          <el-button size="small" @click="entryResume" type="primary"
            >手动录入</el-button
          >
        </div>
      </div>
      ...
      <div v-else>
        ...
        <div class="resume-list mt-3" v-if="total > 0">
          <el-card
            class="box-card mb-3"
            v-for="applyResume in pageResult.list"
            :key="applyResume.id"
          >
            <div
              slot="header"
              class="clearfix text-small"
              style="line-height: 40px; padding-top: 10px"
            >
              <el-popover
                v-if="
                  applyResume.tagVos &&
                  applyResume.tagVos[0] &&
                  applyResume.tagVos[0].tagName
                "
                class="popover"
                placement="top-start"
                width="200"
              ...
              <div class="float-right">
                ...
                <el-button
                  ...
                  v-if="
                    applyResume.applyStatus !== 5 &&
                    applyResume.applyStatus !== 6
                  "
                >
                  不合适
                </el-button>
            ...
            <b-media
              @click="handleShowResume(applyResume)"
            ...
            >
              <template v-slot:aside>
                <el-badge
                  is-dot
                  class="item"
                  :hidden="applyResume.applyStatus !== 1"
                >
              ...
              <b-media-body>
                <h6>
                  ...
                  <span class="ml-4 text-muted text-small">
                    <span v-if="applyResume.resume.maxResumeEdu"
                      >{{ applyResume.resume.maxResumeEdu.degree.name }} /</span
                    >
                    {{ applyResume.resume.age }}岁 /
                    {{ applyResume.resume.curPlace }}
                  </span>
                </h6>
                <div
                  class="tag-group"
                  v-if="
                    applyResume.resume.resumeSkillList &&
                    applyResume.resume.resumeSkillList.length !== 0
                  "
                >
                  <el-tag
                    size="small"
                ...
                <el-row :gutter="12" class="mt-4">
                  <el-col
                    :span="12"
                    v-if="
                      applyResume.resume.resumeEduList &&
                      applyResume.resume.resumeEduList.length !== 0
                    "
                  >
                    <h6>教育经历</h6>
                    <div
                  ...
                  <el-col
                    :span="12"
                    v-if="
                      applyResume.resume.resumeExpList &&
                      applyResume.resume.resumeExpList.length !== 0
                    "
                  >
                    <h6>工作经历</h6>
                    <div
                    ...
                    >
                      <span
                        ...
                        {{ exp.startTime }} - {{ exp.finishTime }}</span
                      >
                    </div>
                  </el-col>
                </el-row>
        ...
        <div v-else class="text-center" style="line-height: 200px">
          暂无简历
        </div>
        <pagination
          v-show="total"
      ...
      <div
        class="mask"
        v-if="reviewDrawerVisible && activeApplyResume"
        ...
        <transition name="slide-fade">
          ...
          >
              ...
              <el-row type="flex" class="row-bg" justify="space-between">
                <el-col class="mt-4 ml-4">
                  <el-button
                    ...
                    v-if="
                      activeApplyResume.applyStatus === 1 ||
                      activeApplyResume.applyStatus === 2
                    "
                  >
                    通过初筛
                  </el-button>
                  ...
                  <el-button
                    ...
                    v-if="
                      activeApplyResume.applyStatus !== 5 &&
                      activeApplyResume.applyStatus !== 6
                    "
                  >
                    不合适
                  </el-button>
            ...
            <div class="resume-drawer-body pl-4 pr-4 pb-4">
              <ResumeView
                :resumeId="activeApplyResume.resume.id"
                class="mt-3"
              ></ResumeView>
            </div>
          </div>
        ...
        <el-timeline class="dialog-content-container">
          ...
          >
            <el-card>
              ...
              <i
                class="el-icon-circle-check note-icon"
                v-if="item.edit"
                @click="onNoteUpdate(item)"
              ></i>
              <i
                class="el-icon-circle-close note-icon"
                v-if="item.edit"
                @click="onNoteCancel(item)"
              ></i>
              <i
                :class="[
                  'el-icon-edit',
                  'note-icon',
                  { 'note-icon-hidden': !item.edit },
                ]"
                @click="onNoteEdit(item)"
              ></i>
              <i
                :class="[
                  'el-icon-close',
      ...
      >
        ...
        <div class="title_container">
          简历上传完成前请勿关闭或刷新本网站（网站内页面仍可继续浏览使用），上传解析完成后将以notification/email提醒
        </div>
        <el-upload
          class="upload-pdf"
          drag
        ...
        >
          ...
          <div class="el-upload__tip" slot="tip">
            ...
            <div class="d-flex align-items-center">
              <el-progress
                :percentage="fileUpdateStatus.progress"
                :show-text="false"
                class="flex-grow-1 m-2"
              ></el-progress>
              <span
                >简历上传{{ fileUpdateStatus.current }} /
                {{ fileUpdateStatus.total }}</span
              >
            </div>
            <div class="d-flex align-items-center">
              <el-progress
                :percentage="fileParseStatus.progress"
                :show-text="false"
                class="flex-grow-1 m-2"
              ></el-progress>
              <span
                >简历解析{{ fileParseStatus.current }} /
                {{ fileParseStatus.total }}</span
              >
            </div>
          </div>
        </el-upload>
      ...
      <el-dialog title="提示" :visible.sync="dialogVisible5" width="40%">
        <div>
          若您关闭此弹窗，简历上传及解析将在后台进行，完成后将以notification/email提醒，届时请刷新简历库页面
        </div>
...
export default {
  ...
  methods: {
    ...
    getNoteList(jobApplyId) {
      this.$axios
        .get("/usercorporate/comment/list", {
          params: { jobApplyId },
        })
        .then((data) => {
    ...
    handleSuccess(response, file, fileList) {
      ...
      this.fileUpdateStatus.current = fileList.filter(
        (file) => file.status === "success"
      ).length;
      this.fileUpdateStatus.progress = parseInt(
        (
          (this.fileUpdateStatus.current * 100) /
          this.fileUpdateStatus.total
        ).toFixed(0)
      );
      ...
      this.$axios
        ...
        .then((data) => {
          ...
          this.fileParseStatus.progress = parseInt(
            (
              (this.fileParseStatus.current * 100) /
              this.fileParseStatus.total
            ).toFixed(0)
          );
          if (
            !this.dialogVisible3 &&
            this.fileParseStatus.current === this.fileParseStatus.total
          ) {
            this.$notify({
              title: "成功",
